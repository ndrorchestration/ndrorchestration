...
import { orbitSnapshot as initialSnapshot, GateState } from '../data/orbit';
...
const Orbit: NextPage = () => {
  ...
  async function refresh() {
    ...
    try {
      ...
      setLive({
        sourceFreshness: payload.live?.sourceFreshness,
        currentHead: payload.live?.github?.currentHead,
        fetchedAt: payload.live?.fetchedAt,
  ...
  return <>
    <Head>
      <title>ORBIT — Evidence Observer</title>
      <meta name="description" content="Read-only evidence observability and reconciliation for DGAF/PDMAL." />
    </Head>
    <main className="shell">
      <nav><a href="/">← NDR AI Systems</a></nav>
      <header className="topbar">
        <div>
          <span className="eyebrow">ORBIT / OBSERVER-ONLY</span>
          <h1>Evidence Observatory</h1>
          <p>Read-only reconciliation of DGAF evidence, provenance, blockers, and claim state. ORBIT does not grant freeze, authorization, or empirical authority.</p>
        </div>
